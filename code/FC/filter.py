def main():
	...
	# --
	# new
	content = '\t'.join([s, p, o] + etc) + '\n'
	gbk_content = content.encode('utf-8')
	...
	with open(output + '/result-scored.tsv', 'w+') as o_file:
		for s, p, o, etc, label, score, expl in result:
			...
			# --
			print("Score of ", score,  "is assigned based on unrecognizable relation "+ p)
	...
	with open(output + '/result-explained-A.tsv', 'w+') as o_file:
		for s, p, o, etc, label, score, expl in result:
			...
			# --
			# print('\t'.join([s, p, o] + etc + [label, score, expl]) + '\n')
